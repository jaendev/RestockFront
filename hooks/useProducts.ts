import { getCantLowStockProducts, getCantProducts, getProducts, getProductsById } from "@/services/productService";
...
export function useProducts(productId?: number) {
  ...
  const [cantLowStockProducts, setCantLowStockProducts] = useState<CantProducts>()
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [productsData, cantProductsData, cantLowStockData] = await Promise.all([
          getProducts(),
          getCantProducts(),
